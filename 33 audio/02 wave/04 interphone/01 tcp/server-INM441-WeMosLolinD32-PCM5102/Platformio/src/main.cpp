...
const char* hostnameLocal="interServer"; //Nombre del dispositivo local
...
void taskSending(void *param) {
  ...
  while(1) { //Bucle infinito
    ...
    if(ipRemote!=0) { //Si tenemos identificado al otro dispositivo...
...
void loop() {
  ...
  //Si la dirección obtenida es distinta a la que teníamos...
  if(ip!=ipRemote) {
    ipRemote=ip; //La actualizamos
    digitalWrite(pinLed,!(ip!=0)); //Led encendido si la ip es válida
  } 
  delay(3000);
}
