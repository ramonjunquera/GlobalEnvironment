/*
  ...
  Material: M5Stack Atom Echo, micrófono externo SPM1423

  Descripción:
  Es importante asegurarse que la conexión del micrófono sea correcta (conectores
  metidos hasta dentro). Si quedan flojos, el micrófono se desconectará continuamente.
*/
...
const char* hostnameLocal="interClientEcho"; //Nombre del dispositivo local
const char* hostnameRemote="interServer"; //Nombre del dispositivo remoto
...
void error() {
  while(1) {
    led.draw({0,0,255}); delay(300); //Led azul
    led.draw(); delay(300); //Led apagado
  }
}
...
void taskReceiving(void *param) {
  ...
  while(1) { //Bucle infinito
    //Si no hay ningún cliente conectado...comprobamos si hay alguno
    if(!client.connected()) client=server.available();
    if(client.connected()) { //Si hay algún cliente conectado...
      if(client.available()) { //Si hay datos pendientes de ser recibidos...
        ...
        while(bytesReceived<bytesPerBlock && client.connected()) {
          //Cuando no se reciben datos, client.read() devuelve -1
          int bytesReceivedNow=client.read(&buffer[bytesReceived],bytesPerBlock-bytesReceived);
          //Si se han recibido datos...se tienen en cuenta
          if(bytesReceivedNow>0) bytesReceived+=bytesReceivedNow;
        }
        //Añadimos el bloque a la cola de reproducción. Si no hay sitio, se pierde.
        xQueueSend(qPlaying,buffer,0);
      }
    }
...
void taskSending(void *param) {
  ...
  while(1) { //Bucle infinito
    ...
    if(wifiConnected && ipRemote!=0) { //Si tenemos conexión wifi e identificado al otro dispositivo...
      //Si no tenemos conexión con server...lo intentamos
      if(!client.connected()) client.connect(ipRemote,8266);
      //Si tenemos conexión...enviamos el bloque de muestras convertidas
      if(client.connected()) client.write(buffer,bytesPerBlock); 
    }
    //Si no tenemos conexión wifi, o no tenemos identificada la dirección ip del otro dispositivo,
    //o no podemos conectar con su servidor, perdemos el bloque de muestras
  }
}
...
void setup() {
  ...
  led.draw({200,0,0}); led.draw({200,0,0}); //Led en rojo = sin conexión con dispositivo remoto
...
void loop() {
  ...
  if(anyChange) {
    if(wifiConnected && ipRemote!=0) led.draw({0,200,0}); //Si la conexión es correcta...led en verde
    else led.draw({200,0,0}); //Si la conexión falla...led en rojo
  }
  delay(3000);
}
