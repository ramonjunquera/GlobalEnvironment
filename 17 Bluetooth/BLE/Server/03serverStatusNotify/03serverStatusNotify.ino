/*
  ...
  La diferencia es que ahora el cliente recibe los cambios de estado en tiempo real.
  Esto es debido a que cuando se cambia el estado, enviamos una notificación a todos
  los clientes que estén conectados.
  Para que las notificaciones de una característica funcionen, es necesario añadirle
  un descriptor del tipo "Client Characteristic Configuration".
  Este tipo de descriptor permite que el cliente pueda activar/desactivar las
  notificaciones.
  El identificador (UUID) de este descriptor es 0x2902. Por eso su librería de gestión
  tiene de nombre BLE2902.h

  En el ejemplo actual, cuando definimos la característica, sólo incluimos la propiedad
  de notificación (BLECharacteristic::PROPERTY_NOTIFY). Esto permite recibir el valor
  sólo por notificaciones (siempre que las activemos). Pero no podemos solicitar el
  valor por nosotros mismos.
  Para tener las dos posibilidades, deberíamos incluir ambas propiedades:
    BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY
    
  Resultado:
  La característica muestra el estado del led en tiempo real
*/
