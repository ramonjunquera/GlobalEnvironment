...
void test1() {
  ...
  for(byte y=0;y<8;y++) {
    for(byte x=0;x<8;x++) {
      leds.v->drawPixel(x,y,leds.v->getColor(99,99,99)); //Encendemos el led procesado en gris
...
//Colores planos
void test2() {
  leds.v->clear(leds.v->getColor(99,0,0)); //Rojo
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(0,99,0)); //Verde
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(0,0,99)); //Azul
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(99,99,0)); //Amarillo
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(99,0,99)); //Magenta
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(0,99,99)); //Cyan
  leds.draw();
  delay(1000);
  leds.v->clear(leds.v->getColor(99,99,99)); //Blanco
  leds.draw();
  delay(1000);
  leds.v->clear(); //Negro
  leds.draw();
...
void test3() {
  ...
  spr.print("/5x7.fon","Hola mundo!",spr.getColor(0,99,0)); //En verde
    ...
    for(byte p=0;p<28;p++) { //Recorremos todas las posiciones del círculo...
      ...
      for(byte steps=0;steps<13;steps++) { //Número de pasos por posición...
        ...
        for(byte i=0;i<28;i++) { //Recorremos todas la posiciones del círculo...
          ...
          leds.v->drawPixel(x,y,leds.v->getColor(levels[i],0,0));
        }
        leds.draw();
        //Dimming
        for(byte i=0;i<28;i++) {
...
void test8() {
  ...
  while(millis()<maxTime) {
    if(random(10)==3) leds.v->drawPixel(random(8),random(8),leds.v->getColor(255,255,255)); //Una de cada 10 veces...pintamos un pixel de blanco
    //Dimming
    for(byte i=0;i<20;i++) {
      for(byte y=0;y<8;y++) {
        for(byte x=0;x<8;x++) {
          byte r,g,b;
          leds.v->getColor(leds.v->getPixel(x,y),&r,&g,&b); //Descomponemos color en canales
          if(r>0) r--;
          leds.v->drawPixel(x,y,leds.v->getColor(r,r,r)); //Todos los canales son iguales
        }
      }
      leds.draw();
    }
...
void test9() {
  ...
  for(byte i=0;i<85;i++) {
    fireColor[i]=leds.v->getColor((byte)(i*3),0,0); //R
    fireColor[i+85]=leds.v->getColor(255,(byte)(i*3),0); //G
    fireColor[i+170]=leds.v->getColor(255,255,(byte)(i*3)); //B
  }
  fireColor[255]=leds.v->getColor(255,255,255);
