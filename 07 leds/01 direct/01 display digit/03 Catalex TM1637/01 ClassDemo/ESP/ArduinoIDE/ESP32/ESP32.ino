/*
  ...
  Material adicional: breadboard, cables, placa ESP32, display de led de 4 dígitos tipo reloj con chip TM1637

  Descripción:
  Aprenderemos a utilizar al chip TM1637 que viene integrado en el display de 4 dígitos de tipo reloj de la marca Catalex.
  ...
  Arduino no tiene una librería nativa para gestionar este chip, así que crearemos una para utilizarla en el ejemplo.
  La descripción detallada del funcionamiento del TM1637 se incluirá dentro de los fuentes de la librería.
  ...
  Mostraremos en el display el tiempo que ha transcurrido desde el último reinicio de la placa en formato MM:SS.
  ...
  Gestión de errores.
  Tanto la librería como el programa tiene en cuenta la gestión de errores. Si se produce cualquier error en cualquier comando
  puede ser detectado. En nuestro caso, cuando se detecta un error, se resetea el display. Con esto conseguimos que si alguna
  vez se envía algo con error, se mostrará el dígito mal escrito en el display, pero sólo durante un máximo de un segundo
  que es el tiempo de refresco.
...
const byte pinCLK=14;
const byte pinDIO=13;
