...
uint16_t yellow=display.getColor(255,255,0); //Color amarillo

void setup() {
  //Inicialización de display para M5Stack Fire
  display.begin(33,27,14,32);

  display.line(120,40,120,35,0xFFFF); //Marca de Norte
  display.printOver("/10x15.fon","N",0xFFFF,115,15); //N de Norte
...
void loop() {
  ...
  if(azimut>=0) { //Si el azimut es válido...
    ...
    txt.print("/10x15.fon",String(azimut),0xFFFF); //Lo llenamos con el valor texto del valor del azimut
    ...
    v.circle(120,120,119,0xFFFF); //Dibujamos la esfera
