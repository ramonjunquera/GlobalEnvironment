...
RoJoXPT2046 ts;
//Variables globales
uint32_t palette[8]; //Paleta de colores
uint32_t currentColor; //Color actual
...
void setup() {
  ...
  palette[0]=display.getColor(0,0,0); //negro
  ...
  //Tienen un tamaño de 40x40, pero el perímetro queda reservado para el recuadro de selección
  for(uint16_t i=0;i<8;i++) display.block(1+i*40,441,38,38,palette[i]);
  //Ponemos un recuadro blanco al color seleccionado
  display.rect(currentColorIndex*40,440,40,40,display.getColor(255,255,255));
    ...
    if(x>=0) {
      ...
      if(y>440) {
        ...
        if(index!=currentColorIndex) {
          ...
          display.rect(currentColorIndex*40,440,40,40,0);
