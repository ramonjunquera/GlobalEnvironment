...
class SpiArduino {
	...
	//Asigna la frecuencia de comunicación en función del divisor de reloj.
	...
	void setClockDivider(arduinoSpiClockDivider divider) {
		...
		if(!enabled) return; //Si no se ha activado el protocolo SPI...terminamos
	...
	void setHwCs(bool status) {
		if(status) { //Si hay que activarlo...
			chipSelect(CS0); //...sólo quedará el CS0 (por defecto)
			pinMode(7,OUTPUT); //El CS1 (pin 7) quedará libre
		} else { //Si hay que desactivarlo...
			chipSelect(CS_NONE); //...desactivamos el control por hardware
			//Y ambos pines CS quedarán libres
			pinMode(8,OUTPUT); //CS0
			pinMode(7,OUTPUT); //CS1
		}
	}
