/*
  ...
  Nota: La configuración con Wemos Lolin32Lite no utiliza lector SD porque este
  dispositivo no tiene salida de 5V al que conectarlo.
  ...
  Nota 2:
    ...
    5V, no podemos utilizar lectores SD de 5V
  Nota 3:
    Con Lolin32 es obligatorio añadir al circuito una resistencia de 
    10 Kohm entre el pin 2 y GND, para poder transferir datos.
  Nota 4:
...
void Test1() {
  ...
  display.block(0,0,32,32,display.getColor(255,0,0)); //Rojo
  display.block(32,0,32,32,display.getColor(0,255,0)); //Verde
  display.block(64,0,32,32,display.getColor(0,0,255)); //Azul
  display.block(0,32,32,32,display.getColor(255,255,0)); //Amarillo
  display.block(32,32,32,32,display.getColor(255,0,255)); //Magenta
  display.block(64,32,32,32,display.getColor(0,255,255)); //Cyan
  delay(1000);
}
...
void Test3() {
  ...
  for(uint16_t x=0;x<display.xMax();x+=10) display.line(0,0,x,display.yMax()-1,color);
  //Dibujamos líneas azules
  color=display.getColor(0,0,255); //azul
  for(uint16_t x=0;x<display.xMax();x+=10) display.line(0,display.yMax()-1,x,0,color);
...
void Test4() {
  ...
  uint32_t borderColor=display.getColor(255,255,255); //white
  uint32_t fillColor=display.getColor(0,0,255); //blue
  //Dibujamos una matriz de 5x5 rectángulos con borde y relleno
  for(int x=0;x<50;x+=10)
    for(int y=0;y<50;y+=10)
      display.rect(x,y,9,9,borderColor,fillColor);
  ...
  for(int y=-40;y<70;y+=30)
    display.rect(70,y,30,29,borderColor,fillColor);
...
void Test7() {
  ...
  display.rect(0,0,50,50,display.getColor(255,255,255),display.getColor(0,0,255));
...
void Test8() {
  ...
  display.clear();
  //Dibujamos un rectángulo de borde blanco y relleno azul
  display.rect(0,0,8,8,display.getColor(255,255,255),display.getColor(0,0,255));
  //Lo copiamos 5 veces
...
void Test19() {
  ...
  //Dibujamos líneas verdes
  uint32_t color=display.getColor(0,255,0); //Verde
  for(uint16_t x=0;x<display.xMax();x+=10) sprite.line(0,0,x,display.yMax()-1,color);
  //Dibujamos líneas azules
  color=display.getColor(0,0,255); //azul
  for(uint16_t x=0;x<display.xMax();x+=10) sprite.line(0,display.yMax()-1,x,0,color);
...
void Test20() {
  ...
  sprite.block(10,10,30,20,sprite.getColor(255,0,0));
...
void Test24() {
  ...
  uint32_t backColor=display.getColor(0,0,64); //Azul oscuro
  //Imprimimos fuera de pantalla
  ...
  sprite.print(F("/10x15d.fon"),F("1234567890"),color,0,sprite.getColor(255,255,255));
