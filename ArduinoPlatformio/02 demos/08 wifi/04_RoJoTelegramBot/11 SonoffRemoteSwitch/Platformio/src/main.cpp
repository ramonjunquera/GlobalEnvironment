/*
  ...
  Configuración de memoria en Arduino IDE:
    ESP8266 Generic. 1M (64K LittleFS)
 */

//Si se define la siguiente constante se compilará para Sonoff S20
//Si no se define, se hará para un ESP-12E
//#define SONOFF

#include <Arduino.h>
#include <ESP8266WiFi.h> //Librería para gestión de wifi
#include <LittleFS.h> //Librería para gestión de archivos
...
String defaultKeyb="[[\"/on\",\"/off\"],[\"/status\",\"/users\"]]"; //Teclado por defecto
  ...
  const byte pinLed=D0; //Pin del led integrado D0=16
...
void setStatus() {
  ...
  File f=LittleFS.open(statusRelayFile,"w");
...
void readStatus() {
  ...
  File f=LittleFS.open(statusRelayFile,"r");
  //Si no se pudo abrir...
  if(!f) {
    //..se crea
    f=LittleFS.open(statusRelayFile,"w");
    //Se guarda el estado "apagado", por defecto
    f.write(0);
    //Se cierra
    f.close();
    //Se vuelve a abrir como sólo lectura
    f=LittleFS.open(statusRelayFile,"r");
  }
