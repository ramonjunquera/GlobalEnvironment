/*
  ...
  Su máximo está definido en el archivo FreeRTOSConfig.h, en la constante
  configMAX_PRIORITIES. El máximo valor permitido sería configMAX_PRIORITIES-1.
  En ESP32, por defecto, configMAX_PRIORITIES=25
  Por lo tanto el rango de valores posibles para la prioridad de una tarea
  es [1,24].
  La prioridad de una tarea aumenta con su valor.
  La prioridad 0 la tiene la tarea idle y no deberíamos utilizarla.
  Es recomendable asignar a una tarea el valor más bajo posible. Así
  permitiremos que las tareas prioritarias no se vean afectadas.
*/
...
void setup() {
  ...
  BaseType_t answer; //Guardará el resultado de la creación de la tarea
