/*
  ...
  Notas:
  - La carpeta data debe existir en la carpeta del ejecutable con su contenido para su correcto funcionamiento.
...
void Test13()
{
  ...
  //Leemos su contenido desde un archivo
  backSprite.load(F("data/mickey.spr"));
...
void Test14()
{
  ...
  RoJoSprite ballMask;
  //Leemos su contenido desde un archivo
  ballMask.load(F("data/ballmask.spr")); //Su tamaño es x=30,y=30,pages=4
  //Coordenadas
  byte x=0;
  byte y=0;
  //Deltas
...
void Test15()
{
  ...
  RoJoSprite mySprite;
  //Leemos su contenido desde un archivo
  mySprite.load(F("data/mickey.spr"));
  //Dibujamos el sprite de fondo sobreescribiendo lo que haya
...
void Test16()
{
  ...
  mySprite.save(F("data/mick2.spr"));
  //Borramos el sprite utilizado
  mySprite.clean();
  //Creamos un nuevo sprite
  RoJoSprite mySprite2;
  //Leemos su contenido desde el archivo
  mySprite2.load(F("data/mick2.spr"));
  //Borramos la pantalla
  display.videoMem->clear();
  ...
  mySprite2.clean();
  //Borramos el archivo
  SPIFFS.remove(F("data/mick2.spr"));
...
void Test18()
{
  ...
  font.print(F("data/5x7d.fon"),F("20171101"),&textSprite);
  //Lo mostramos
  display.videoMem->drawSpritePage(0,0,&textSprite,4);

  //Utilizaremos otra fuente más grande
  //Si no hemos podido crear el sprite de texto...terminamos
  if(!font.print(F("data/10x15d.fon"),F("20171101"),&textSprite)) return;
  //Lo mostramos
  display.videoMem->drawSpritePage(0,2,&textSprite,4);
  
  //Creamos otro texto más grande que la pantalla 
  //Si no hemos podido crear el sprite de texto...terminamos
  if(!font.print(F("data/10x15d.fon"),F("1234567890123456"),&textSprite)) return;
...
void Test19()
{
  ...
  if(!font.print("data/5x7d.fon","2017",&normalSprite)) return;
...
void Test20()
{
  ...
  if(!font.print(F("data/7x11.fon"),F("Hello world!"),&textSprite)) return;
  //Lo mostramos
  display.videoMem->drawSpritePage(0,0,&textSprite,4);
  //Creamos otro texto
  //Si no podemos crear el sprite de texto...terminamos
  if(!font.print(F("data/7x11.fon"),F("Good morning"),&textSprite)) return;
  //Lo mostramos
  display.videoMem->drawSpritePage(0,2,&textSprite,4);
  ...
  //Creamos el sprite con el texto
  //Si no podemos crear el sprite de texto...terminamos
  if(!font.print(F("data/5x7.fon"),F("Hello world!"),&textSprite)) return;
  //Lo mostramos
  display.videoMem->drawSpritePage(0,4,&textSprite,4);
  //Creamos otro texto
  //Si no podemos crear el sprite de texto...terminamos
  font.print(F("data/5x7.fon"),F("Good morning"),&textSprite);
  //Lo mostramos
  ...
  //Creamos el sprite con el texto
  //Si no podemos crear el sprite de texto...terminamos
  if(!font.print(F("data/3x5.fon"),F("Hello world!"),&textSprite)) return;
  //Lo mostramos
  display.videoMem->drawSpritePage(0,6,&textSprite,4);
  //Creamos otro texto
  //Si no podemos crear el sprite de texto...terminamos
  if(!font.print(F("data/3x5.fon"),F("Good morning"),&textSprite)) return;
  //Lo mostramos
